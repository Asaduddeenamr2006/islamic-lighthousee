...
export default function TafsirPanel({ surah, onClose }: TafsirPanelProps) {
  ...
  const loadTafsir = async () => {
    setLoading(true);
    try {
      const surahInfoRes = await fetch(`https://api.quran.com/api/v4/chapters/${surah.id}?language=ar`);
      let totalAyahs = surah.verses_count;
      if (surahInfoRes.ok) {
        const sData = await surahInfoRes.json();
        totalAyahs = sData.chapter.verses_count;
      }

      const quranRes = await fetch(`https://api.quran.com/api/v4/verses/by_chapter/${surah.id}?language=ar&fields=text_uthmani&per_page=${totalAyahs}`);
      const quranData = await quranRes.json();
      let quranAyahs = quranData.verses.map((v: any) => ({
        numberInSurah: v.verse_key ? parseInt(v.verse_key.split(':')[1]) : 0,
        text: v.text_uthmani || '',
      }));

      let tafsirAyahs: any[];
      if (selectedSource === 'ar.ibnkathir') {
        const ikRes = await fetch(`https://cdn.jsdelivr.net/gh/spa5k/tafsir_api@main/tafsir/ar-tafsir-ibn-kathir/${surah.id}.json`);
        const ikData = await ikRes.json();
        tafsirAyahs = ikData.ayahs.map((a: any) => ({ numberInSurah: a.number, text: a.text }));
      } else {
        const tRes = await fetch(`https://api.alquran.cloud/v1/surah/${surah.id}/${selectedSource}`);
        const tData = await tRes.json();
        tafsirAyahs = tData.data.ayahs;
      }

      const bismillah = 'بِسْمِ ٱللَّهِ ٱلرَّحْمَـٰنِ ٱلرَّحِيمِ';
      if (surah.id !== 1 && surah.id !== 9 && quranAyahs.length > 0) {
        ...
        if (tafsirAyahs[0]?.text?.includes(bismillah)) {
          tafsirAyahs = [...tafsirAyahs];
          tafsirAyahs[0] = { ...tafsirAyahs[0], text: tafsirAyahs[0].text.replace(bismillah, '').trim() };
        }
      }

      const result = tafsirAyahs.map((t: any, i: number) => ({
        numberInSurah: t.numberInSurah,
        text: t.text,
        ayahText: quranAyahs[i]?.text || '',
      }));
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#151820] flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between px-3 sm:px-6 py-3 sm:py-4 bg-[#1a1d26] border-b border-white/10 flex-shrink-0">
        <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
          ...
          >
            {TAFSIR_SOURCES.map(source => (
              <option key={source.id} value={source.id} className="bg-[#1a1d26]">
                {source.name}
              </option>
            ))}
          </select>
