...
import { useSwipeWithFeedback } from '../hooks/useSwipe';
...
export default function QuranContent({ 
  ...
  const swipeHandlers = useSwipeWithFeedback({
    onSwipeLeft: handleSwipeLeft,
    onSwipeRight: handleSwipeRight,
    threshold: 60,
    enabled: isMobile,
    onSwipeProgress: (progress) => {
      setSwipeDx(progress * 60);
    },
  });
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <div className="w-10 h-10 border-3 border-t-transparent rounded-full animate-spin" style={{ borderColor: accentColor, borderTopColor: 'transparent' }} />
  ...
  if (!surah) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
        ...
        <h2 className="text-2xl font-bold text-white mb-2">القرآن الكريم</h2>
  ...
  const swipeTransform = isMobile ? `translateX(${swipeDx}px)` : undefined;
  const swipeOpacity = isMobile ? 1 - Math.abs(swipeDx) / 300 : undefined;

  if (focusMode) {
    return (
      <div className="flex-1 overflow-y-auto" {...(isMobile ? swipeHandlers : {})}>
        <div 
  ...
  return (
    <div className="flex-1 overflow-y-auto p-3 sm:p-4 lg:p-6" {...(isMobile ? swipeHandlers : {})}>
      <div className="max-w-4xl mx-auto">
        <div
          className="rounded-xl sm:rounded-2xl border border-white/10 bg-[#0a0a0a] overflow-hidden shadow-xl transition-transform duration-200"
          style={isMobile ? { transform: swipeTransform, opacity: swipeOpacity } : undefined}
        >
          <div className="flex items-center justify-between px-3 sm:px-6 py-3 sm:py-4 border-b border-white/5">
            <div className="flex items-center gap-2 sm:gap-3">
              <span className="px-3 sm:px-4 py-1 sm:py-1.5 rounded-full text-xs sm:text-sm font-bold text-white" style={{ background: accentColor }}>
