...
export default function AudioPlayer({
  ...
  return (
    <div className="h-16 sm:h-20 bg-[#0a0a0a]/90 backdrop-blur-md border-t border-white/5 flex items-center px-2 sm:px-4 gap-2 sm:gap-3 relative">
      {/* Reciter Dropdown */}
      {showReciterList && (
        <div className="absolute bottom-full right-2 sm:right-4 mb-1 bg-[#0a0a0a] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50 w-64 max-h-64 overflow-y-auto">
          {reciters.map(reciter => (
            <button
              ...
              style={{
                background: selectedReciter === reciter.id ? `${accentColor}20` : undefined,
                color: selectedReciter === reciter.id ? accentColor : undefined,
              }}
            >
              {reciter.name}
            </button>
      ...
      <div className="flex items-center gap-1 sm:gap-1.5 flex-shrink-0">
        ...
        <button
          ...
          style={{
            background: playMode !== 'none' ? `${accentColor}20` : undefined,
            color: playMode !== 'none' ? accentColor : undefined,
          }}
        ...
        <button
          onClick={onPlayPause}
          className="w-10 h-10 sm:w-11 sm:h-11 rounded-full flex items-center justify-center text-white transition-all duration-200 hover:scale-105 flex-shrink-0"
