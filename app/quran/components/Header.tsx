...
export default function Header({ title, onMenuToggle, onTafsirClick, onSettingsClick, onFocusModeToggle, focusMode = false, colorScheme, fontSize, onFontSizeChange, onColorChange }: HeaderProps) {
      ...
      <div className="flex items-center gap-0.5 sm:gap-1 relative">
        ...
        {onFocusModeToggle && (
          <button
            ...
            className={`w-8 h-8 flex items-center justify-center rounded-lg transition-colors lg:hidden ${focusMode ? 'text-white bg-white/10' : 'text-white/70 hover:text-white hover:bg-white/5'}`}
            title="وضع التركيز"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
